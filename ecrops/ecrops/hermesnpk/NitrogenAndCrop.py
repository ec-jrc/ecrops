...
class NitrogenAndCrop:
    ...
    def runstep(self, status):
        ...
        WURM = int(math.floor((g.WURZMAX) * (g.WUMAXPF / g.WUMAXPF))) #davide: check this istruction. In the original code there is a /10. What does it mean?
        ...
        for i in range(1, g.WURZ+1):
            ...
            if i > 1 :
                status.hermesnitrogen.WULAE2[index] = abs(status.hermesnitrogen.WULAE[index]-status.hermesnitrogen.WULAE[index-1]) / (math.pow(status.hermesnitrogen.WRAD[index], 2) * math.pi)/DZ
            else:
                status.hermesnitrogen.WULAE2[index] = abs(status.hermesnitrogen.WULAE[index]) / (math.pow(status.hermesnitrogen.WRAD[index], 2) * math.pi)/DZ
        ...
        for i in range(0,3): #WUMM = N contained in dead roots is ditributed in NFOS/NAOS in the first 3 layers. Each one gets 1/6 of it
        ...
        for index in range (0,minim):
            if index+1 < 11:
                ...
                status.hermesnitrogen.MASS[index] = g.TP[index] * (status.hermestransport.C1 [index] / (g.WG[index] * DZ )) #TP[z] =water uptake in layer Z (SoilDepthLayer)
                status.hermesnitrogen.TRNSUM = status.hermesnitrogen.TRNSUM + g.TP[index]*(status.hermestransport.C1 [index]/(g.WG[index] * DZ))
                D[index] = 2.14 * (status.hermesnitrogen.params.AD * math.exp(g.WG[index]*10)) / g.WG[index]
                status.hermesnitrogen.DIFF[index] = (D[index] * g.WG[index] * 2 * math.pi * status.hermesnitrogen.WRAD[index]
                                                     * (status.hermestransport.C1 [index]/1000/g.WG[index] - .000014) * math.sqrt(math.pi*g.WUDICH[index])) \
                                                    * g.WUDICH[index] * 1000
        ...
        for index in range (0,minim):
            if DTGESN > 0 :
                if status.hermesnitrogen.TRNSUM >= DTGESN :
                    status.hermesnitrogen.PE[index] = DTGESN * status.hermesnitrogen.MASS[index] / status.hermesnitrogen.TRNSUM
                else:
                    if DTGESN-status.hermesnitrogen.TRNSUM < status.hermesnitrogen.SUMDIFF:
                        status.hermesnitrogen.PE[index] = status.hermesnitrogen.MASS[index] + (DTGESN - status.hermesnitrogen.TRNSUM ) \
                                                          * status.hermesnitrogen.DIFF[index]/status.hermesnitrogen.SUMDIFF
                    else:
                        status.hermesnitrogen.PE[index] = status.hermesnitrogen.MASS[index] + status.hermesnitrogen.DIFF[index]
        ...
        if g.WUMAS > g.WUMALT :
                if g.FRUCHT == "ZR " or g.FRUCHT == "K  ":
                    if (g.OBMAS - g.OBALT + g.WUMAS - g.WUMALT) > 0 :
                        g.WUGEH = (g.WUMALT*g.WUGEH + ((g.WUMAS - g.WUMALT) / (g.OBMAS + g.WORG[3] - g.OBALT + g.WUMAS - g.WUMALT) * status.hermesnitrogen.SUMPE)) / g.WUMAS

                else:
                    if (g.OBMAS - g.OBALT + g.WUMAS - g.WUMALT) > 0 :
                        g.WUGEH = (g.WUMALT*g.WUGEH + (g.WUMAS-g.WUMALT)/(g.OBMAS-g.OBALT+g.WUMAS-g.WUMALT)*(status.hermesnitrogen.SUMPE+NFIX)) / g.WUMAS
